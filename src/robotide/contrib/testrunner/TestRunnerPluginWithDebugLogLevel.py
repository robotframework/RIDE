...
class TestRunnerPluginWithDebugLogLevel(Plugin):
    """A copy of the Test Runner plugin Debug log level"""
    ...
    def __init__(self, application=None):
        ...
        self._min_log_level_number = LOG_LEVELS['DEBUG']
    ...
    def _register_actions(self):
        run_action_info = ActionInfo("Tools", "Run Tests with Debug", self.OnRun, None,
                                     "F9", getBugIconBitmap(), "Run the selected tests with Debug", position=8)
        self._run_action = self.register_action(run_action_info)
        stop_action_info = ActionInfo("Tools", "Stop Test Run with Debug", self.OnStop, None,
                                      "CtrlCmd-F9", ImageProvider().TOOLBAR_STOP, "Stop a running test with Debug",position=9)
    ...
    def _build_runner_toolbar(self):
        ...
        toolbar.AddLabelTool(ID_RUN,"Start", getBugIconBitmap(), shortHelp="Start robot",
