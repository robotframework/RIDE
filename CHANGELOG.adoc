...
- Added RIDE.app to install in Applicatons on MacOS
    * Not ready yet, because it cannot run ``robot``. This is because is calling by shell command.
    * For now it is static. Points to a 3.7 Python installation.
    ª Users should edit the command in MacOS/RIDE to the correct path
    ª Users can associate files .robot to open with RIDE (but opens last file edited)
