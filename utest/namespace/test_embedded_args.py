...
class TestEmbeddedArgs(unittest.TestCase):
    ...
    def test_no_embedded_arguments(self):
        with pytest.raises(TypeError):
            EmbeddedArgsHandler(KWMock('no embedded args'))

    def test_embedded_args(self):
        args = EmbeddedArgsHandler(KWMock('add user ${user} to db'))
        assert args.name_regexp.match('add user test to db')
        assert not args.name_regexp.match('add user test to somewhere else')

    def test_several_args(self):
        args = EmbeddedArgsHandler(KWMock('${user} should ${foo} and ${bar}'))
        assert args.name_regexp.match('john should eat and drink')
        assert not args.name_regexp.match('this should not match')

    def test_custom_variable_regexp(self):
        args = EmbeddedArgsHandler(KWMock('Say hello to ${user:[A-C]+}'))
        assert args.name_regexp.match('Say hello to ABC')
        assert not args.name_regexp.match('Say hello to ABCD')
